      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div variants={fadeInUp} className="mb-10 md:mb-14">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            <h2 className="font-sora font-black text-3xl sm:text-4xl lg:text-5xl text-text-light dark:text-text-dark leading-[1.05] tracking-tight max-w-md">
              What it takes to be part of our network
            </h2>
            <p className="font-nunito text-sm text-muted-light dark:text-muted-dark max-w-xs leading-relaxed">
        ...
        <motion.div variants={stagger}>
          {EXPECTATIONS.map(({ icon: Icon, title, body }, i) => (
            <motion.div
              key={title}
              variants={fadeInUp}
              className="flex items-start gap-5 py-5 border-b border-border-light dark:border-border-dark"
            >
              {/* Index + icon */}
              <div className="shrink-0 flex items-center gap-3 w-32 sm:w-40">
                <span className="text-f10 font-mono text-muted-light dark:text-muted-dark w-5 text-right">
                  {String(i + 1).padStart(2, '0')}
                </span>
                <div className="w-8 h-8 flex items-center justify-center border border-border-light dark:border-border-dark bg-surface-light dark:bg-surface-dark shrink-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="font-nunito text-sm text-muted-light dark:text-muted-dark leading-relaxed">{body}</p>
              </div>
            </motion.div>
          ))}
        </motion.div>
