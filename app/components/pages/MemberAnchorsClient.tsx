...
import AnchorCard from '@/app/components/cards/AnchorCard'
...
const MemberAnchorsClient = ({ data }) => {
  const session = useSession()
  const [searchQuery, setSearchQuery] = useState('')
  const [statusFilter, setStatusFilter] = useState('all')

  const filteredAnchors = data
    ?.filter((anchor) => {
      const matchesSearch =
        searchQuery === '' ||
      ...
      >
        ...
        <div className="sticky top-0 z-20 flex flex-col md:flex-row gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              {getAnchorStatusOptions(data).map((option) => (
...
export default MemberAnchorsClient
