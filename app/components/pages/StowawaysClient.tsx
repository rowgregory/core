...
import StowawayCard from '@/app/components/cards/StowawayCard'
import InfoBanner from '@/app/components/common/InfoBanner'

const StowawaysClient = ({ data }) => {
  const [searchQuery, setSearchQuery] = useState('')
  const [statusFilter, setStatusFilter] = useState('all')

  const filteredStowaways = data
    ?.filter((user) => {
    ...
    <div className="bg-gray-900 min-h-[calc(100vh-68px)]">
      ...
      >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 xs:gap-3 sm:gap-4">
          {getApplicationStatusOptions(data)
...
export default StowawaysClient
